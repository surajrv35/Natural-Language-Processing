...
class RNN(nn.Module):
    ...
    def train(self, input, target):
        ...
        return total_loss.item(), outputs   # use total_loss.data[0] for version 0.3.0_4 and below, .item() for 0.4.0
    ...
    def eval(self, input):
        ...
        for ivec in input:
            ...
            _, hidden_state = self.encoder.forward(Variable(ivec), hidden_state)
            print("in input-- ",ivec, hidden_state)
        sentence = []
        input = self.sos
        print("inputd.data[0,1]-- ",input.data[0, 0])
        # Decoder
        while (input.data[0, 0] != 1):
            print("in while-- ",input.data[0, 0])
